(function (context, localforage) {
	var event = new Event('network-ready'),
        ...
        window.addEventListner("network-ready", function () {
            for (i = 0; i < queue.length; i++) {
                    processRequest(queue[i](0), queue[i](1), queue[i](2), false);
            }
        });
			...
			errorCallback = function(){
				if (typeof callback === 'function') {
					callback(xhr.response, xhr.status, xhr);
                                ...
                                if (!critical) {
                                        // Request failed so add it back to the queue
                                        addNonCriticalRequest(url, data, callback);
                                }
			};
